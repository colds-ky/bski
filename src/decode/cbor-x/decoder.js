      ...
      case 0x1f: 
        ...
        switch(majorType) {
          ...
          case 4: // array
            ...
            return majorType == 4 ? array : majorType == 3 ? array.join('') :
              // @ts-ignore
              Buffer.concat(array)
          case 5: // map
            let key
...
function shortStringInJS_singleChar() {
  let a = src[position++]
  if ((a & 0x80) > 1) {
    position -= 1
    return
...
function shortStringInJS_2or3(length) {
  let a = src[position++]
  let b = src[position++]
...
function shortStringInJS_longer_8ormore(length, a, b, c, d) {
...
function shortStringInJS_longer(length) {
  let a = src[position++]
  let b = src[position++]
  let c = src[position++]
  let d = src[position++]
  ...
  if (length < 6) {
    ...
    return shortStringInJS_longer_8ormore(length, a, b, c, d);
  }
}
...
function shortStringInJS(length) {
  if (length < 4) {
    if (length < 2) {
      if (length === 0) return ''
      else return shortStringInJS_singleChar()
    } else {
      return shortStringInJS_2or3(length)
    }
  } else {
    return shortStringInJS_longer(length)
  }
}
...
const packedTable = (read) => {
  throw new Error('packed table is not supported')
...
currentExtensions[PACKED_REFERENCE_TAG_ID] = (data) => { // packed reference
  throw new Error('packed references not supported')
  // if (!packedValues) {
